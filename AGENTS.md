...
20. **计费时段以官方定价页为准，并且一律换算到 UTC 再判定。**
    现行规则（2026-09-10 起）：**高峰 = 工作日 01:00–04:00 与 06:00–10:00 UTC**，其余时间
    一律半价（含午休与整个周末）；优惠价 = 高峰价的一半。旧的「每天 00:30–08:30 错峰优惠」
    已经作废，别照那个写。规则放在 `$script:PeakUtcBlocks`（UTC 分钟数），改完必须同步改
    `-LogicTest` 里的边界用例。**不许把本地时刻写死**（例如「北京时间 9 点到 12 点」）：
    那会让别的时区的机器全算错。
