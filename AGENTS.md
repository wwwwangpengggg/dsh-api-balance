...
一个 DSH（DeepSeek Harness）桌面插件：启动后在桌面角落常驻一个置顶小窗，三页显示
DeepSeek API 余额、今日累计消耗的 token、今日消费，以及当前是高峰还是半价的计费时段。
...
assets/balance-window.ps1   小窗全部实现（单文件，约 1800 行）
...
15. **token 折叠语义必须与 DSH 的 token-meter 保持一致**：`assistant/chunk` 与
    ...
    **对外报的是「今日」（跨多次开关机累加），不是「本次开机」。** 宿主只在运行期间看得见
    事件，所以落盘要按天合并（`usageDayPayload` + 文件里的 `boots` 分条记录，每条只由产生
    它的那次开机更新，并发才不会互相抹掉）；`usageDayKey` 一变就整份重开。
    `usageDayKey` 必须与 PowerShell 的 `Get-SpendingDayKey` 一致——日界在 JS 与 PS 各实现
    一遍，守卫办法是两边测试写死同一批边界期望值（08:00 换天 / 07:59 不换 / 起点 0 即自然
    日 / 起点 20 点时次日 19:00 仍同一天），改一边就立刻变红。
    升级路径也要照顾：旧结构文件（没有 `dayKey`）按 `updatedAt` 是否落在当天之内决定认领
    还是丢弃（`adoptLegacyUsage`）——不认领的话用户一重启就看到「今日 0」，而他明明已经用
    了一整天（真发生过）。
    文件里的 `total` 与 `totalText` 是窗口唯一读的两个字段，改结构时别动这两个键。
...
16. **换页用「点金额区域」，不要改成左右滑动。** 卡片本身靠拖动移动位置，横向滑动会和拖拽
    ...
    页数由 `$script:PageCount` 说了算（现在是 3）：点一下 = `Flip-Page` = 往后一页、到头绕回
    第 1 页；页码点与菜单项都从 `PageCount` 推出来，别再写死 2 或 3。
    拖动那条边界也要守：**只有越过 3px 才开始挪窗口**。以前是「先挪、再判断算不算拖动」，
    于是手抖 1–3px 会把窗口一点点挪走——实测连点八次每次偏 3px。
17. **`-LogicTest` 是 PowerShell 侧唯一的单测入口，别删也别绕开。**「余额下降才算消费、
    ...
    改记账逻辑必须同步改那里的用例。
    **自检失败必须吵。** 窗口自检最怕的不是失败，而是「卡在某一步却没有任何输出」：拿不到
    单实例互斥量时以前会静默 `exit 0`，测试只能看到「输出里没有 PASS」，完全查不出原因。
    现在这类路径一律打 `SELFTEST FAIL: 原因`，自检主体也整段包 try/catch 把异常转成问题
    条目。加新的自检步骤时请沿用这个规矩。
...
20. **计费时段以官方定价页为准——每次动它之前先把原文取回来，别照记忆写**（我漏过一次：
    只照了更早的旧文字，漏掉「excluding Chinese public holidays」，于是中秋假期的周五被
    误判成高峰，被用户当场抓到）。并且一律换算到 UTC 再判定。
    现行规则（2026-09-10 起，2026 年 9 月又补了节假日排除）：
    **高峰 = 周一至周五 01:00–04:00 与 06:00–10:00 UTC，法定节假日除外**；其余时间一律半价
    （含午休、周末、以及法定节假日的整天）；优惠价 = 高峰价的一半。
    - 「法定节假日」由 `$script:ChineseHolidays`（**北京时间日期**）判定，数据取自国务院
      放假安排。**必须看当天是否节假日，不能只看星期几**——2026 年中秋 9/25 就是周五，
      整天优惠。
    - **「调休上班」的周末不算高峰**：官方只写了 Monday through Friday。
    - **每年国务院公布次年安排后必须补表**（通常 11 月）。没收录的年份按「不排除节假日」
      处理：宁可显示成高峰，也不误报成半价，错要错在不影响用户花钱的那一边。
      用户也可以用 `config.holidays` 临时补（逗号分隔的 yyyy-MM-dd）。
    - 规则常量在 `$script:PeakUtcBlocks`，节假日表在 `$script:ChineseHolidays`；改完必须同步
      改 `-LogicTest` 的边界用例。**不许把本地时刻写死**（例如「北京时间 9 点到 12 点」）：
      那会让别的时区的机器全算错。
    第 3 页只是**提示**，不参与计费——任何文案都不能说成「按此价格计费」。
21. **两个 PowerShell 陷阱，改小窗几乎一定会再撞上：**
    - **`[string]` 参数的 `$null` 会被存成空串。** 若 `$script:Xxx` 里 `Xxx` 是脚本参数且声明
      为 `[string]`，类型约束会把赋进去的 `$null` 变成 `''`，于是 `-ne $null` 永远为真。
      `-NowOverride` 就是这么炸的：`Get-Now` 返回空串，绑参数时才报错，而且**只在窗口模式
      暴露、逻辑自检全绿**（那次排查绕了很久）。存「有没有值」请另起一个无类型约束的变量，
      并用 `-is` 判定。
    - **`[ordered]` 哈希的整数索引器是「第几项」，不是「键 N」。** `$pageItems[2] = ...` 会去
      设置还不存在的第 3 项，当场抛 `ArgumentOutOfRange`，脚本在建**菜单**的时候就死了，
      现象是「窗口完全不出现」。有序列表请用数组（`@()` + `+=`）。
