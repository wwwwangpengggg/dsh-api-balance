...
17. **`-LogicTest` 是 PowerShell 侧唯一的单测入口，别删也别绕开。**「余额下降才算消费、
    充值不计入、换天只归零计数而保留基准线、换币种整份重开」这套口径写在 `.ps1` 里，
    Node 侧调不动；`tests/window-script.test.mjs` 跑它并断言 `LOGICTEST PASS`。
    改记账逻辑必须同步改那里的用例。
18. **今日消费是估算值，不是账单。** 余额接口没有明细（生态里同类工具也只能这么算），只能按
    余额减少量倒推。三条口径，改之前先想清楚：
    - 充值不计入、不抵扣；
    - 换币种整份重开；**换天只把计数归零，基准线必须保留**——连基准线一起清掉会丢掉小窗
      关着那段时间的花费，后果是「用户明明花了几十块、卡上只有几毛」（v0.2.1 就是这样，
      被用户当场抓到）；
    - 只有小窗运行期间才数得到，它没跑那段的消费无从得知。
    任何文案都不能把它说成精确消费额；页面上因此写的是「按余额减少量估算」。
19. **「一天」是「8 点起算的 24 小时」，不是自然日**（起点由 `dayStartHour` 配，默认 8）。
    ...
    另外：官网的「今日消费」按自然日计费，窗口不同，两个数字**本来就不相等**——不要为了让
    它们相等去改这里的口径，那是拿估算值去凑账单。
