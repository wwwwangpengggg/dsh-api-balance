...
│ 09:44:39 已更新 · 每 60s …   │   │ 24 小时后归零 · 充值不计入   │
│                          ● ○ │   │                          ○ ● │
└──────────────────────────────┘   └──────────────────────────────┘
            第 1 页：余额                    第 2 页：今日消费
...
- **换天或换币种一律从零重开。** 于是 DSH 关着的那段时间花掉的钱**不会**被记进来
  （下次开机第一次取数时，上一天的基准线已经被清掉了）。
...
| 今日消费一直是 ¥0.00 | 多半正常：余额没减少就不会有消费。可看 `spending.json` 里的 `lastBalance` 是否与当前余额一致；越过起算时刻后从零重开也是预期行为 |
