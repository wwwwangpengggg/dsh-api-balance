 ...
 * 认领「旧结构」文件里的数字。
 *
 * v0.3.0 及之前的 usage.json 没有 dayKey，只能记「本次开机」——一个可能横跨多天的数字。
 * 升级后第一次落盘若一律当成新的一天，用户刚重启就会看到「今日 0」，而他明明已经用了一整
 * 天。所以按「旧文件最后一次写入落在哪一天」来判断：
 *
 *   - 写在**当前这一天之内** → 那个数字就属于今天，认领成一条记录，接上而不是从零开始；
 *   - 写在更早的日子 → 那是别的日子的量，丢掉（返回 null）。
 *
 * 只在升级那一次会用到；文件一旦变成新结构，这里就永远返回 null。
 * @param previous - 读到的旧载荷。
 * @param dayKey - 当前这一天的键（`usageDayKey`）。
 * @returns 可交给 `usageDayPayload` 的载荷，或 null（没有可认领的历史）。
 */
export function adoptLegacyUsage(previous, dayKey) {
  if (previous === null || typeof previous !== 'object') return null
  // 已经是新结构：按 dayKey 正常处理，这里不插手。
  if (typeof previous.dayKey === 'string' && previous.dayKey !== '') return null

  const writtenAt = Date.parse(typeof previous.updatedAt === 'string' ? previous.updatedAt : '')
  // dayKey 形如 2026-09-14T08:00，不带时区，按规范就是本地时间——正是我们要的。
  const dayStart = Date.parse(dayKey)
  if (!Number.isFinite(writtenAt) || !Number.isFinite(dayStart) || writtenAt < dayStart) return null

  return {
    dayKey,
    boots: {
      'pre-upgrade': {
        total: toCount(previous.total),
        input: toCount(previous.input),
        output: toCount(previous.output),
        cacheRead: toCount(previous.cacheRead),
        cacheWrite: toCount(previous.cacheWrite),
