...
import { adoptLegacyUsage, createUsageTracker, foldUsage, usageDayKey, usageDayPayload, usageSnapshot } from './usage.js'
...
export function applyUsageTracking(ctx, cfg, log, deps = {}) {
  ...
  const flush = () => {
    ...
    try {
      const at = now()
      const dayKey = usageDayKey(at, cfg.dayStartHour)
      // 每次都重读：另一个实例可能刚更新过它那一条，重读才不会把它的数字抹掉。
      const previous = readUsageFile(cfg.usagePath)
      // 升级路径：旧结构（没有 dayKey）里那个数字如果是今天写的，就地认领——否则用户刚重启
      // 就会看到「今日 0」，而他明明已经用了一整天。
      const carried = adoptLegacyUsage(previous, dayKey) ?? previous
      writeJsonFile(cfg.usagePath, usageDayPayload(carried, usageSnapshot(tracker), dayKey, bootKey, at))
    } catch (error) {
